...
export default function Sauces({ onSauceChange, onOrder }: SaucesProps) {
    ...
    return (
        <section
            ref={sectionRef}
            id="sosevi"
            className="min-h-screen w-full flex flex-col justify-center px-4 md:px-20 py-20 relative z-10 overflow-hidden transition-colors duration-700"
        >
            {/* Dynamic Ambient Background Glow - uses sauce color */}
            <div
                className="absolute top-1/2 left-0 md:left-1/2 w-[60vw] h-[60vw] md:w-[800px] md:h-[800px] blur-[120px] rounded-full -translate-y-1/2 -translate-x-1/2 -z-10 pointer-events-none mix-blend-screen animate-glow-pulse transition-all duration-700"
                style={{ backgroundColor: sauce.color, opacity: 0.25 }}
            />

            {/* Section Title */}
            <div className={`mb-12 text-center reveal-hidden ${isVisible ? 'reveal-up' : ''}`}>
            ...
            {/* Sauce Selector - each button in its own color */}
            <div className={`flex justify-center gap-2 mb-16 overflow-x-auto pb-2 no-scrollbar reveal-hidden ${isVisible ? 'reveal-scale stagger-2' : ''}`}>
                {(Object.keys(SAUCES) as Array<keyof typeof SAUCES>).map((key) => (
                    <button
                        key={key}
                        onClick={() => handleSelect(key)}
                        className={`
                            px-6 py-2 rounded-full font-bold text-sm tracking-widest uppercase transition-all duration-300
                            ${activeSauce === key
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 w-full max-w-7xl mx-auto items-center">
                {/* Visual Side (Image) */}
                <div className="order-1 md:order-2 flex justify-center h-[50vh] relative">
                    <img
                        src={sauce.image}
                        alt={sauce.name}
                        className={`
                        ...
                        style={{
                            ...
                            ...(isAnimating ? {} : { dropShadow: `0 0 40px ${sauce.color}44` })
                        }}
                    />
                </div>
                ...
                <div className="order-2 md:order-1 relative z-20">
                    ...
                    >
                        <div className={`flex items-baseline gap-4 mb-6 reveal-hidden ${isVisible ? 'reveal-left stagger-3' : ''}`}>
                            <h3 className="text-5xl font-black">{sauce.name}</h3>
                            <span className="text-xl text-gray-400 font-normal">{sauce.subtitle}</span>
                        </div>

                        {/* Accent line in sauce color */}
                        <div
                            className={`w-16 h-1 mb-8 ${isVisible ? 'animate-line-grow stagger-4' : 'scale-x-0'}`}
                            style={{ backgroundColor: sauce.color }}
                        />

                        <p className={`text-xl text-gray-300 mb-10 leading-relaxed font-light reveal-hidden ${isVisible ? 'reveal-left stagger-4' : ''}`}>
                            {sauce.description}
                        </p>

                        <div className={`grid grid-cols-2 gap-8 mb-12 border-t border-white/10 pt-8 reveal-hidden ${isVisible ? 'reveal-up stagger-5' : ''}`}>
                            <div>
                            ...
                            <div>
                                ...
                                {isTrio && (sauce as any).oldPrice && (
                                    <span className="block text-gray-500 line-through text-lg">{(sauce as any).oldPrice} RSD</span>
                                )}
                                <span className="text-3xl font-bold">{sauce.price} <span className="text-sm font-normal text-gray-500">RSD</span></span>
                            </div>
                            <div className="col-span-2">
                                <span className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Najbolje uz</span>
                                <span className="text-white">{sauce.bestWith}</span>
                            </div>
                        </div>

                        <button
                            onClick={() => onOrder(isTrio ? 'Trio Paket (Sva tri sosa)' : `Sos ${sauce.name}`)}
                            className={`group w-full md:w-auto text-white font-black text-lg py-4 px-12 rounded-full transition-all hover:scale-105 flex items-center justify-center gap-2 reveal-hidden ${isVisible ? 'reveal-scale stagger-6' : ''}`}
                            style={{
