...
function ContactItem({ item, index }: { item: typeof CONTACT_ITEMS[0]; index: number }) {
    ...
    return (
        <a
            ref={ref}
            href={item.href}
            className={`flex items-center gap-6 group p-6 rounded-2xl border border-white/5 
...
export default function Contact() {
    ...
    return (
        <section id="kontakt" className="relative py-20 bg-black text-white">
            <div className="max-w-7xl mx-auto px-6 md:px-20">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                    ...
                    <div ref={headingRef}>
                        ...
                        <h2 className={`text-5xl md:text-7xl font-black mb-8 leading-tight reveal-hidden ${headingVisible ? 'reveal-left stagger-1' : ''}`}>
                            JAVI SE <br />
                            <span className="text-gray-600">AKO SMEŠ.</span>
                        </h2>
                        <p className={`text-gray-400 text-xl font-light max-w-md reveal-hidden ${headingVisible ? 'reveal-left stagger-2' : ''}`}>
